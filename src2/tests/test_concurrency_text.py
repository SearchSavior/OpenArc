...
def unload_model(model_name: str) -> Dict[str, Any]:
    ...
    return http_post("/openarc/unload", payload)
...
def main() -> None:
    ...
    model_a = {"name": "Qwen3-1.7B-int8_asym-ov-GPU1", "device": "GPU.1"}
    model_b = {"name": "Qwen3-1.7B-int8_asym-ov-GPU2", "device": "GPU.2"}
    ...
    try:
        ...
        # Unload models after test completion
        print(f"\nUnloading models...")
        try:
            print(f"Unloading model {model_a['name']}...")
            unload_response_a = unload_model(model_a["name"])
            print(f"  Response: {unload_response_a}")
        except Exception as e:
            print(f"  Failed to unload {model_a['name']}: {e}")

        try:
            print(f"Unloading model {model_b['name']}...")
            unload_response_b = unload_model(model_b["name"])
            print(f"  Response: {unload_response_b}")
        except Exception as e:
            print(f"  Failed to unload {model_b['name']}: {e}")
