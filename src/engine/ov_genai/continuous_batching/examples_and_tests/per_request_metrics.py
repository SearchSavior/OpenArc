"""
Per-request metrics for OpenVINO GenAI's ContinuousBatchingPipeline.

Each prompt submitted to the pipeline gets its own RequestTracker, which
records first/last token times for every sequence the request produces.
That gives us isolated TTFT, TPOT, and throughput numbers per inference
even when requests are batched together.
"""
...
class SequenceTiming:
    """Timing for one sequence within a request (a request may emit several)."""
...
class GenerationMetrics:
    """Computed metrics for a single request."""
    input_tokens: int
    new_tokens: int
    ttft_ms: float
    tpot_ms: float
...
class RequestTracker:
    """Wraps a single GenerationHandle and computes its metrics."""

    def __init__(self, handle: GenerationHandle, input_len: int):
        self.handle = handle
    ...
    def poll(self) -> None:
        """Pull any newly-generated tokens off the handle and record timing."""
        for seq_id, output in self.handle.read().items():
            seq = self.sequences.setdefault(seq_id, SequenceTiming())
            seq.record(len(output.generated_ids))
    ...
    def compute_metrics(self) -> GenerationMetrics:
        ...
        # Across sequences: earliest first-token wins for TTFT,
        # latest last-token defines the decode window.
        ...
        decode_tokens = max(0, new_tokens - 1)  # first token counted in TTFT
...
def build_generation_config(max_new_tokens: int = 5096) -> ov_genai.GenerationConfig:
...
def submit_prompts(
    ...
    for req_id, prompt in enumerate(prompts):
        ...
        trackers.append(RequestTracker(handle, count_tokens(tokenizer, prompt)))
    return trackers
...
def drive_pipeline(
    ...
    """Step the pipeline until every request is finished, polling each tick."""
