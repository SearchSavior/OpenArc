...
``runtime_config`` is deliberately absent: it is a free-form dict of OpenVINO
properties with no contract to generate flags from, and stays a JSON string.
...
# scheduler_config is a load-time field (a sibling of the config blocks in
# config.yaml, and a field on ModelLoadConfig), not a request-default block.
...
def _scalar(annotation: Any) -> Optional[Any]:
    """Reduce an annotation to the bool/int/float/str/Enum it carries.

    ``Optional[X]`` and ``X | None`` unwrap to ``X``. Returns None when the
    field is not expressible as a single CLI flag.
    """
...
def _help_text(field: str, info: Any) -> str:
    description = _WHITESPACE.sub(" ", info.description or "").strip()
    return description or field
...
def exposed_fields(dest: str) -> List[str]:
    ...
    return [
        ...
        if name not in REQUEST_ONLY_FIELDS and name not in LOAD_CONFIG_FIELDS
    ]


def field_registry() -> Dict[str, Tuple[Any, str]]:
    """Map every CLI-exposed field to (scalar type, help text).
    ...
    for dest in _GROUP_ORDER:
        ...
        for name in exposed_fields(dest):
            if name in registry:
                continue
            scalar = _scalar(contract.model_fields[name].annotation)
            if scalar is None:
                raise ConfigOptionError(
                    f"Cannot expose contract field '{dest}.{name}' "
                    f"({contract.model_fields[name].annotation!r}) as a CLI flag. "
                    f"Teach config_options.py to render it or exclude it via "
                    f"REQUEST_ONLY_FIELDS."
                )
            registry[name] = (scalar, _help_text(name, contract.model_fields[name]))
    return registry
...
def build_config_options() -> List[click.Option]:
    ...
    for name, (scalar, help_text) in field_registry().items():
        ...
        if scalar is bool:
            params = [f"--{kebab_name}/--no-{kebab_name}"]
            attrs: Dict[str, Any] = {"default": None, "help": help_text}
        else:
            params = [f"--{kebab_name}"]
...
def _group_label(dest: str) -> str:
    ...
    if dest == SCHEDULER_DEST:
        return f"{dest}   ·   all model types, written to load_config"
...
def resolve_config_values(
    ...
    Returns:
        ``(scheduler_config, blocks)`` where scheduler_config is written under
        ``load_config`` and blocks map block name -> validated authored keys.
